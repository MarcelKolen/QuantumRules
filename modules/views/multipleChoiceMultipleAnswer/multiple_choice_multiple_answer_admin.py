...
from modules.forms import MultipleChoiceMultipleAnswerForm, MultipleChoiceMultipleAnswerChoicesForm
...
def render_adding_in_category(request, typeID, gameID, categoryID):
    ...
    """
    template_name = 'modules/multipleChoiceMultipleAnswerModule/admin/add_in_category.html'

    choices_formset = formset_factory(MultipleChoiceMultipleAnswerChoicesForm, extra=1)

    # Populate form with POST data for errorhandling
    form = MultipleChoiceMultipleAnswerForm(request.POST or None)
    form_choices = choices_formset(request.POST or None)

    context = {'all_module_items': MultipleChoiceMultipleAnswer.objects.order_by('name'),
               'typeID': typeID,
               'gameID': gameID,
...
def render_adding(request, typeID):
    ...
    """
    template_name = 'modules/multipleChoiceMultipleAnswerModule/admin/add.html'
...
def render_edit(request, typeID, itemID):
    ...
    template_name = 'modules/multipleChoiceMultipleAnswerModule/admin/edit.html'

    try:
        multiplechoice_item = MultipleChoiceMultipleAnswer.objects.get(ID=itemID)
    except MultipleChoiceMultipleAnswer.DoesNotExist:
        messages.add_message(request, messages.ERROR,
                             'Er is een fout opgetreden aanpassen van een Multiple Choice Multiple Answer Item!')
        messages.add_message(request, messages.DEBUG,
                             f'The exception <i>\'MultipleChoiceMultipleAnswer.DoesNotExist\'</i> occurred on id {itemID}')
        return redirect('game:adminpanelModules')

    choices_formset = modelformset_factory(MultipleChoiceMultipleAnswerChoices, MultipleChoiceMultipleAnswerChoicesForm, extra=1, can_delete=True)

    # Populate form with POST data and database instances
    form = MultipleChoiceMultipleAnswerForm(request.POST or None, instance=multiplechoice_item)
    form_choices = choices_formset(request.POST or None, queryset=MultipleChoiceMultipleAnswerChoices.objects.filter(related_question=multiplechoice_item))
...
def add(request):
    ...
    """
    choices_formset = modelformset_factory(MultipleChoiceMultipleAnswerChoices, MultipleChoiceMultipleAnswerChoicesForm, extra=1)


    form = MultipleChoiceMultipleAnswerForm(request.POST)
...
def edit(request, itemID):
    ...
    choices_formset = modelformset_factory(MultipleChoiceMultipleAnswerChoices, MultipleChoiceMultipleAnswerChoicesForm, can_delete=True)

    form = MultipleChoiceMultipleAnswerForm(request.POST, instance=multiplechoice_item)
    form_choices = choices_formset(request.POST, queryset=MultipleChoiceMultipleAnswerChoices.objects.filter(related_question=multiplechoice_item))
    ...
    if form.is_valid():
        if form_choices.is_valid():
            ...
            # If there is at least one valid answer possibility, edit the item with new data
            if atleast_one_correct is True:
                item = form.save()
                choice_instance = form_choices.save(commit=False)
                marked_for_delete = form_choices.deleted_forms
