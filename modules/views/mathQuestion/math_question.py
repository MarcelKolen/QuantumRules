...
def render_module(request, gameID, itemID):
    ...
    Renders math question
    """

    return basic_module_render(request, gameID, itemID,
                               'modules/mathQuestionModule/math_question.html',
                               'Math Question')
...
def handle_input(input, gameID, itemID, raw_request_data=None, raw_socket_data=None):
    ...
    Math questions do not have any user input other than pressing a button to go to the next part, so the item will always be satisfied.
    """
